...
constructor(@AppContext context: Context, forecastRepo: ForecastRepo, private val darkSkyApi: DarkSkyApi): BaseForecastViewModel<LocationDetailsMvvm.View>(context, forecastRepo), LocationDetailsMvvm.ViewModel {
    ...
    override fun loadLocalForecast(zipCode: String) {
        val forecastFromDb = forecastRepo.getByField("zip", zipCode, true)
        if (forecastFromDb != null) {
            update(forecastFromDb)
        }
    }

    override fun reloadForecastFromRemote(zipCode: String, lat: String, lng: String) {
